...
| `estimatedMomentumTime` | Used to schedule waits between ceremonies. |
...
| Field | Drives |
| --- | --- |
| `administrator` | If this signer's producer address matches, it runs in administrator mode and does not join TSS ceremonies. |
...
| `tssNonce` | Nonce included in every signed message, so a signature cannot be replayed. |
...
| `partyTimeout`, `keyGenTimeout`, `keySignTimeout` | TSS ceremony timeouts, in seconds. |
...
The full JSON-RPC surface of the bridge embedded contract, its constants and its error strings are documented upstream at [hypercore-team.github.io](https://hypercore-team.github.io/decentralized_bridge/core_contracts/embedded_bridge/api.html).
