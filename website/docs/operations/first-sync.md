...
A progress line every 25 ranges, then a completion line. At 3 requests per second and 2,000 blocks per query, mainnet from a 2023 deployment takes roughly 15 to 20 minutes.
...
The same range is retried with backoff from 2 seconds to 2 minutes. After 12 consecutive failures the pass stops and resumes from the same cursor at the next refresh, 3 minutes later. The cursor **never** advances past a range that did not complete, so nothing is skipped by a failure.
...
The orchestrator subscribes to new logs and refreshes the subscription every 3 minutes with a short catch-up sync. `latestUpdateHeight` in [`getStatus`](../health-api.md) should then follow the head. Live events wait in `queues/` until they have `confirmationsToFinality` confirmations, are validated the same way, and only then enter the signing pool.
